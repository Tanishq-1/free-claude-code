...
> Date: 2026-09-26 (sync #11 — see the LAST SYNC block below)
...
> **LAST SYNC (sync #11 recorded 2026-09-26):** upstream base `faeec6ba` → **`ba4b9341`** (18 commits, 90 files +5165/−997: PyPI publishing + release-version automation, AGPL-3.0-only relicence, `requires-python ==3.14.7` and `.python-version` REMOVED, `fcc-doctor`, **Cheaper Inference provider (#1899)**, fixed cloud endpoints #1891, openai_api sampling params #1883). Local main ff-synced to `ba4b9341` (lineage verified via `merge-base --is-ancestor`, exit 0). **7 non-frozen branches rebased onto `ba4b9341`, all 0-behind, all green (§8 sync #11). §4 VERSIONING RULE IS NOW OBSOLETE — READ BELOW.** `final` REBUILT on `ba4b9341` (ancestry 9/9; gates green; suite 6272p/18s/**2 known failures**; e2e 195p/1s). **NOTHING PUSHED** (user decision — local only).

> ### ⚠️ §4 SUPERSEDED — manual version bumps are now FORBIDDEN (upstream #1896/#1903)
> The old rule "`pyproject.toml` line 7: bump `version = "X.Y.Z"` + `uv lock` in the same commit" **no longer applies and will FAIL CI.** Upstream switched to `hatch-vcs` with `dynamic = ["version"]` and added `scripts/check_version_policy.py` + the required `version-policy.yml` check, which **rejects any `project.version`** (*"FCC must declare a dynamic version without project.version"*). Consequences: **(1)** delete the `version = …` line, keep upstream's `dynamic = ["version"]` — on every rebase the pyproject conflict is exactly this line; resolve to upstream's form, then `git checkout main -- uv.lock` → `uv lock`. **(2)** The branch-version invariant ("sit ABOVE the base version") is **dead** — versions now come from git tags at release time. **(3)** Release intent comes from the **PR title**: any PR touching `pyproject.toml`/`uv.lock`/`.python-version`/`assets/`/`scripts/`/`src/` MUST be titled `patch: …`/`minor: …`/`major: …`, and a non-release PR must NOT use those prefixes. Verify with `uv run python scripts/check_version_policy.py --base main --head HEAD --title 'minor: …'`. Features ⇒ `minor:`, fixes ⇒ `patch:`.

> ### §7 branch triage changes (sync #11)
> * **`add-lightning-ai` (#1740) and `add-experiential-labs` (#1749) are MERGED upstream** (`613cdaaa`, `0329ac49`) — branches gone, dropped from the active set. Catalog is now **56** on main.
> * `add-telepub-voyage` (§12.1 freeze) and `fix-nim-minimax-post-terminal-framer` (#1742, closed unmerged) remain **frozen at `50559ea0` / `cfa42f81`** — verified byte-identical after sync #11. Both are still merged into `final` at those pins per §9.

> ### ⚠️ Known deliberate trade-off on `final`: 2 failing tests
> `tests/providers/test_sdk_stream_lifecycle.py::test_nim_parser_failure_closes_response_before_retry[messages|responses]` **fail on `final` by decision.** `fix-nim-minimax-post-terminal-framer` holds back `finish_reason` to tolerate post-terminal NIM chunks; upstream later added a newer contract requiring the response to be **closed before a retry**. The two behaviours are mutually exclusive — reverting the fix fails its own 7 tests instead. Its 36 own tests all pass and `minimax` is still a live catalog provider, so the bug is still reachable. **User decision 2026-09-26: keep the fix, accept the 2 failures.** Every other branch and `main` are fully green. To drop it later: rebuild `final` without that branch.
...
# Lockfile (only if dependencies changed — NOT for versions, see §4)
uv lock
...
# ... make changes, run gates + tests, check the version policy (NO version bump — see §4) ...
...
## 4. Versioning — **SUPERSEDED (sync #11). Manual bumps are FORBIDDEN.**

> **Do NOT bump `pyproject.toml` by hand any more.** Upstream (#1896/#1903) moved to `hatch-vcs` + `dynamic = ["version"]` and added a **required CI check** (`.github/workflows/version-policy.yml` → `scripts/check_version_policy.py`) that **fails any PR carrying `project.version`**. Setting `version = "X.Y.Z"` in a branch will now break the `Version policy` check.

1. **No version line in a branch.** `pyproject.toml` must keep `dynamic = ["version"]` and MUST NOT contain `version =`. Versions are derived from git tags when `post-merge.yml` publishes to PyPI. The "branch-version invariant" is dead.
2. **Release intent = the PR title.** A PR touching `pyproject.toml`, `uv.lock`, `.python-version`, or anything under `assets/`, `scripts/`, `src/` is a *release* PR and MUST be titled `patch: …` / `minor: …` / `major: …`; a non-release PR must NOT use those prefixes. Semantics unchanged in spirit:

   | Prefix | When | Example |
   |--------|------|---------|
   | `patch:` | bug fix only | fix crash |
   | `minor:` | new feature/provider | **added a provider** |
   | `major:` | breaking change | remove provider, rename env vars |

3. **Check locally before pushing** (mirrors the CI job):
   ```powershell
   uv run python scripts/check_version_policy.py --base main --head HEAD --title 'minor: Add <feature>'
   # -> "Version policy passed: minor"
   ```
4. When a rebase conflicts in `pyproject.toml`, the conflict is almost always the old `version` line: **resolve to upstream's `dynamic = ["version"]`**, then `git checkout main -- uv.lock` → `uv lock`. `uv.lock`'s editable `free-claude-code` root package must likewise carry no `version` field.
...
8. **`.env.example`** — **SKIPPED since sync #11: upstream deleted this file.** Do not re-create it.
9. **`README.md`** — provider table row.
10. **`smoke/lib/config.py`** — `"<name>": "<name>/<default-model>",` in `PROVIDER_SMOKE_DEFAULT_MODELS`.
11. **Logo asset + attribution** — `src/free_claude_code/api/admin_static/providers/<name>.svg` (or `.png`) plus a row in that folder's `SOURCES.md`. **Both mandatory:** `ProviderDescriptor` now *requires* `website_url` + `logo_filename`, and `e2e/test_provider_modals.py::test_provider_titles_link_to_websites_and_every_logo_loads` asserts every logo actually loads. Reuse the project-authored neutral-glyph convention (see `custom.svg`) if the real brand mark can't be shipped.
12. **Version policy** — **no `pyproject.toml` version edit** (§4). Verify with `uv run python scripts/check_version_policy.py --base main --head HEAD --title 'minor: Add <name>'`; title the PR `minor: …`.
...
### Sync #11 (2026-09-26) — upstream base `faeec6ba` → `ba4b9341` — **7 branches rebased, `final` rebuilt, NOTHING PUSHED**

**Trigger:** upstream advanced 18 commits (90 files, +5165/−997). Main ff-synced to `ba4b9341` after a `merge-base --is-ancestor` lineage check (exit 0 ⇒ genuine fast-forward, no force-push). `uv` auto-provisioned **Python 3.14.7** (upstream now pins `requires-python ==3.14.7` and **deleted `.python-version`**), rebuilding the venv.

**Upstream changes that reshaped the sync:**
* **Release automation (#1896/#1903/#1904) — invalidates §4.** `hatch-vcs` + `dynamic = ["version"]`; `scripts/check_version_policy.py`, `publish_release.py`, `validate_release.py`; required `version-policy.yml` check that **rejects `project.version`**.
* **Licence → AGPL-3.0-only (#1887)**, `fcc-doctor` (#1906), installer checks moved to the opt-in smoke suite (#1908).
* **Cheaper Inference provider (#1899)** — landed in the same catalog/settings/profiles/manifest anchor regions as our provider branches, which is why `add-orca-router` and the telepub merge were "keep-both" conflicts.
* **Fixed cloud endpoints (#1891)** — upstream dropped the `*_DEFAULT_BASE` imports for lightning/experiential/nararoute/tokenrouter; those imports must NOT be re-added during conflict resolution.
* `openai_api` sampling params (#1883), NIM voice by default (#1893), image token pricing (#1895).

**Branch triage (per the user's "keep only branches listed as active/available" instruction):** dropped `add-lightning-ai` (#1740) and `add-experiential-labs` (#1749) — **both merged upstream** (`613cdaaa`, `0329ac49`), branches deleted. `add-telepub-voyage` (§12.1) and `fix-nim-minimax-post-terminal-framer` (#1742) stayed **frozen**; verified byte-identical to their pre-sync pins afterwards.

**Per-branch results — every one rebased, 0-behind, gates green, version-policy compliant:**

| Branch | New SHA | Commits | Policy | Suite | e2e | Notes |
|---|---|---|---|---|---|---|
| `fix-cline-hub-hijack` (#1641) | `85b31b6a` | 1 | `patch` | 6207p/18s | 195p/1s | clean rebase, no conflicts |
| `feat-api-key-rotation` | `2a386dae` | 1 | `minor` | 6222p/18s | 195p/1s | version-only conflict |
| `update-main-fix-issues` (#1744) | `f4048981` | 3 | `patch` | 6218p/18s | 195p/1s | version-only conflict on commit 1 of 3 |
| `add-orca-router` (#1754) | `69189df4` | 1 | `minor` | 6237p/18s | 194p/1s* | **11 files conflicted** — heaviest; keep-both vs cheaperinference |
| `feat-models-metadata` (#1736) | `b335ab82` | 1 | `minor` | 6210p/18s | 194p/1s* | version-only conflict |
| `add-custom-provider` (#1469) | `69acb27d` | 2 | `minor` | 6223p/18s | 195p/1s | version + `test_provider_runtime.py` |
| `fix-ollama-model-discovery` (#1606) | `58f6d818` | 3 | `patch` | 6209p/18s | 195p/1s | **collapsed 10 commits (6 main-merges, 2 no-op) → 3**; architectural re-map |

\* e2e flakes of the `net::ERR_UNSAFE_PORT` class — the e2e fixture picks a random port that sometimes lands on a Chromium-blocked port (5060, 1723, 10080). **Both passed 6/6 and 2/2 in isolation and pass on a clean serial re-run. Not regressions.** When they appear, re-run the single test rather than the suite, and **never run two e2e suites concurrently** (parallel runs collide on ports — this cost real time this cycle).

**Rebase recipe that worked (forks from §3/§8 rails):**
```powershell
$env:GIT_EDITOR='true'                    # prevents the invisible vim.exe hang
git switch <branch>; git rebase main
# pyproject.toml/uv.lock -> keep upstream's dynamic-version form:
git checkout main -- pyproject.toml uv.lock ; uv lock
git add <explicit paths>                   # never -A
git -c core.editor=true rebase --continue
# provider-anchor conflicts -> keep BOTH sides in catalog order; drop imports
#   upstream deleted (LIGHTNING/EXPERIENTIAL/NARAROUTE/TOKENROUTER _DEFAULT_BASE)
uv run python scripts/check_version_policy.py --base main --head HEAD --title 'minor: …'
```

**`final` rebuilt (§9) on `ba4b9341` — ancestry 9/9 verified by exit code.** Merge order: key-rotation → models-metadata → custom → ollama → update-main → cline → **nim** → telepub. Notes:
* `add-custom-provider` conflicted in `providers/openai_chat/provider.py` as documented — combined as `api_key_provider=resolved_api_key_provider` + `credential_optional=…`. **The old `"no-api-key"` SDK-placeholder note in §9 is OBSOLETE** — keyless custom endpoints are now handled by the profile's `credential_optional` flag.
* Telepub's 8 conflicts were **all additive** (distinct provider fields), so §12.1's "stop and ask" trigger did **not** fire. But upstream made `website_url` + `logo_filename` **required** on `ProviderDescriptor`, which telepub (frozen since 2026-08) predates: added both, and created the missing `telepub_voyage.svg` (project-authored neutral glyph, mirroring `custom.svg`) plus its `SOURCES.md` row. **Without the asset, `e2e/test_provider_modals.py::test_provider_titles_link_to_websites_and_every_logo_loads` fails** — the e2e suite *does* assert every logo loads (`naturalWidth > 0`).
* `.env.example` (modify/delete) resolved as **deleted**, matching upstream's deliberate removal.
* Credential-count gotcha (§9): kept the self-computing `assert len(result) == len(keys)` form rather than telepub's hardcoded `== 21` — the computed form cannot go stale when the catalog changes.
* `tests/providers/test_telepub_voyage.py` called `provider._build_request_body(...)`, which upstream moved to the module-level `build_openai_chat_request_body(request, reasoning=…, policy=…)` in `request_policy.py` (surfaced as 4 `ty` errors). Rewired through `OPENAI_CHAT_PROFILES["telepub_voyage"].request_policy`.
* **Catalog = 57** on `final` (56 on main + telepub).

**`final` result:** gates green (769 files formatted, ruff, ty) · suite **6272 passed / 18 skipped / 2 failed** · e2e **195 passed / 1 skipped**. The 2 failures are the accepted `fix-nim` trade-off documented in the header.

**Backups:** `pre-sync11/<branch>` for all 9 branches + `final-backup-20260926` (old `final` `1f8b1fd7`, which was a bare ancestor of main with 0 unique commits — i.e. `final` had been silently emptied before this sync).

**Not done (user decision):** no `git push` to `origin`, no force-push, no PR opened/edited. Every branch is local-only and ready for review.
...
> **⚠️ SYNC #11 AMENDMENTS to the recipe below** (it was written for the pre-#1896 world):
> * The **merge list is now 8, not 11** — drop `add-lightning-ai` and `add-experiential-labs` (both **merged upstream**; branches deleted). The "trio" stacking rule below is **dead**; `add-orca-router` is now the only one of the three left.
> * **Step 4's `provider.py` combination is obsolete** — do NOT use `api_key=resolved_api_key or "no-api-key"`. That branch now uses `credential_optional=True` on the profile, so combine as `api_key_provider=resolved_api_key_provider` + `credential_optional=self._profile.credential_optional`.
> * **Step 5's stacked-count gotcha for `pyproject.toml`/`uv.lock` is obsolete** — there are no versions any more (§4). Resolve those conflicts to upstream's `dynamic = ["version"]` form and regenerate the lock.
> * **`ProviderDescriptor` now requires `website_url` + `logo_filename`**, and the e2e suite asserts every logo asset actually loads — any provider merged into `final` must have its asset + a `SOURCES.md` row.
> * **`.env.example` no longer exists** (upstream deleted it) — resolve that modify/delete as *deleted*.
...
## 10. Current Git State (updated 2026-09-26 — sync #11)

> **Versions are no longer a per-branch thing** (§4) — the "Version" column is gone; release intent is carried by the PR title prefix. **Nothing has been pushed since sync #8** — all sync-#11 SHAs below are local-only.

| Branch | Issue / PR | SHA | Status |
|--------|-----------|-----|--------|
| `main` | — | `ba4b9341` | ff-synced to upstream (18 commits; v6.2.59→~v6.2.71). Local only — **not pushed to fork**. |
| `add-custom-provider` | PR #1469 OPEN (#1230) | `69acb27d` | sync #11: rebased onto `ba4b9341`; `minor:`; gates + 6223 suite + 195 e2e green. 0-behind. |
| `fix-ollama-model-discovery` | PR #1606 OPEN (#1296) | `58f6d818` | sync #11: rebased; `patch:`; gates + 6209 suite + 195 e2e green. 10 commits collapsed to 3. |
| `add-telepub-voyage` | PR #739 URL-share only — NO PR | `50559ea0` | **FROZEN (§12.1)** — unchanged, verified identical to pre-sync pin. Merged into `final` at that pin. |
| `feat-models-metadata` | PR #1736 OPEN (#1168) | `b335ab82` | sync #11: rebased; `minor:`; gates + 6210 suite + e2e green. |
| `fix-nim-minimax-post-terminal-framer` | PR #1742 **CLOSED UNMERGED** (#1734) | `cfa42f81` | **FROZEN** — unchanged. Merged into `final`; causes the 2 accepted failures there. |
| `update-main-fix-issues` | PR #1744 OPEN (#1730) | `f4048981` | sync #11: rebased (3 commits); `patch:`; gates + 6218 suite + 195 e2e green. |
| `add-orca-router` | PR #1754 OPEN (#1751) | `69189df4` | sync #11: rebased, 11-file keep-both conflict vs new `cheaperinference`; `minor:`; gates + 6237 suite + e2e green. |
| `feat-api-key-rotation` | #1301 — NO PR | `2a386dae` | sync #11: rebased; `minor:`; gates + 6222 suite + 195 e2e green. Awaiting user decision on a PR. |
| `fix-cline-hub-hijack` | PR #1641 OPEN (#1590) | `85b31b6a` | sync #11: rebased clean (no conflicts); `patch:`; gates + 6207 suite + 195 e2e green. |
| `final` | — internal, NO PR | see §8 sync #11 | **REBUILT 2026-09-26** on `ba4b9341`, 8 branches merged, ancestry 9/9, catalog 57, gates green, **6272p/18s/2 known failures**, e2e 195p/1s. |
| ~~`add-lightning-ai`~~ | PR #1740 | — | **REMOVED — merged upstream** (`613cdaaa`). |
| ~~`add-experiential-labs`~~ | PR #1749 | — | **REMOVED — merged upstream** (`0329ac49`). |

### Sync #8 state (superseded by sync #11) — kept for history
